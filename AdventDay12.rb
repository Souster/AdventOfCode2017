...
def find_zero(input)
  @total_array = input
  @arrays_containing_zero = []
  @total_array.each do |program|
    split_array = program.split(" ")
    if find_value_in_program("0",split_array )
      add_value(split_array[0], @arrays_containing_zero)
      find_sub_programs(split_array)
    end
    split_array = split_array.uniq
  end
  puts "Containing 0: #{@arrays_containing_zero}"
  puts "Length: #{@arrays_containing_zero.length}"
end

def find_value_in_program(value, arr)
  for i in 1..arr.length - 1
    if arr[i].gsub(",","").eql?(value)
      return true
    end
  end
  return false
...
def find_sub_programs(array_to_search)
  if array_to_search.length >= 3
    for i in 2..array_to_search.length - 1
      value = array_to_search[i].gsub(",", "")
      if !@arrays_containing_zero.include? value
        add_value(value, @arrays_containing_zero)
        program = find_program_in_list(value, @total_array)
        if program
          find_sub_programs(program)
        end
      end
    end
  end
...
def find_program_in_list(number, list)
  for i in 0..list.length - 1
    if list[i].split(" ")[0].eql?(number)
      return list[i].split(" ")
    end
  end
  return nil
...
def find_in_overall(number, list)
  for i in 0..list.length - 1
    if list[i].split(" ")[0].eql?(number)
      return list[i]
    end
...
# Question 2
def find_zero2(input)
  @groups = 0

  for i in 0..1999
    find_groups_for_value(input, i.to_s)
  end
  puts "Groups: #{@groups}"
end

def find_groups_for_value(input,index)
  @total_array = input
  @arrays_containing_zero = []
  input.each do |program|
    split_array = program.split(" ")
    if find_value_in_program(index,split_array )
      add_value(split_array[0], @arrays_containing_zero)
      find_sub_programs(split_array)
    end
    split_array = split_array.uniq
  end
  length = @arrays_containing_zero.count
  if length >= 1
    @groups += 1
  end
  @arrays_containing_zero.each do |number|
    item = find_in_overall(number, input)
    index = input.index(item)
    if index
      @total_array.delete_at(index)
    end
  end
end
