...
def run_instruction(instruction)
  ...
  #5 condition
  #6 value
  options = instruction[0].split(" ")

  if !check_for_register_in_array(options[0])
    @register_array.push(["#{options[0]}", 0])
  end

  if !check_for_register_in_array(options[3])
    @register_array.push(["#{options[4]}", 0])
  end

  if get_value_from_key(options[4]).to_i.send(options[5],options[6].to_i) then
    value = 0
    case options[1]
    when "dec"
      value = get_value_from_key(options[0]).to_i - options[2].to_i
    when "inc"
      value = get_value_from_key(options[0]).to_i + options[2].to_i
    end
    find_and_update(options[0], value.to_s)
  end
end

def get_value_from_key(key)
  @register_array.each do |item|
    if item[0].eql?(key)
      return item[1]
    end
  end
...
def check_for_register_in_array(key)
  @register_array.each do |item|
    if item[0].eql?(key)
      return true
    end
...
run_sequence2([["b inc 580 if bkd > -1"],
["v inc 491 if z >= -1"],
